...
import type { TagRead } from "@/lib/api";
import { createCourseTag } from "@/lib/api";
...
export function TagCreateSheet({
  ...
  async function handleCreate() {
    ...
    setError("");
    try {
      const created = await createCourseTag({ name: trimmed });
      onCreated(created);
      onClose();
      setTagName("");
    } catch (caughtError) {
      ...
      <View style={{ gap: 12 }}>
        ...
        <Pressable
          ...
          style={({ pressed }) => ({
            ...
            opacity: saving || !tagName.trim() ? 0.6 : pressed ? 0.85 : 1
          })}
        >
