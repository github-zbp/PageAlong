        ...
        <View style={{ gap: 22 }}>
          ...
          {continueCourse ? (
            <View style={{ gap: 10 }}>
              ...
              >
                <Text numberOfLines={2} style={{ color: tokens.text, fontSize: 18, fontWeight: "700" }}>{continueCourse.title}</Text>
                <Text style={{ color: tokens.mutedText, fontSize: 13 }}>
                  {formatCourseMeta(continueCourse, locale)}{formatUpdatedDate(continueCourse.updated_at, locale) ? ` · ${formatUpdatedDate(continueCourse.updated_at, locale)}` : ""}
                </Text>
