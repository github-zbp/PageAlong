...
@celery_app.task(name="import_file_for_course")
def import_file_for_course(course_id: str, item_id: str) -> dict[str, str]:
    api_dir = get_api_dir()
    env = os.environ.copy()
    env["PYTHONPATH"] = str(api_dir)
    completed = subprocess.run(
        [get_api_python(), "-m", "app.cli.import_file", item_id],
        cwd=api_dir,
        env=env,
        check=True,
        capture_output=True,
    ...
    return {
        ...
        "item_id": str(payload.get("item_id", item_id)),
