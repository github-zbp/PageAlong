...
def normalize_tags(tags: list[str] | None) -> list[str]:
    ...
    for tag in tags or []:
        ...
        if not normalized or normalized in seen:
            continue
        normalized_tags.append(normalized[:64])
        seen.add(normalized)
...
def course_tags(course: Course) -> list[str]:
    if course.series is not None and not course.series.is_deleted:
        return decode_tags(course.series.tags_json)
    return decode_tags(course.tags_json)
...
def course_matches_tag(course: Course, tag: str | None) -> bool:
    ...
    if not normalized_tag:
        return True
    return normalized_tag in course_tags(course)
...
def list_courses(
    ...
    courses = list(
        db.scalars(
            select(Course)
            .options(selectinload(Course.series))
            .where(
...
def list_series(
    ...
    for series in series_items:
        if not active_series_courses(series):
            continue
...
def update_course_library(
    ...
    if moving_to_fragmented:
        if course.series is not None:
            if tags is None:
                course.tags_json = course.series.tags_json
...
def move_series_to_fragments(db: Session, series: CourseSeries) -> int:
    ...
    for course in courses:
        course.series_id = None
        course.series = None
        course.tags_json = series.tags_json
...
def list_tags(db: Session, user_id: str) -> list[str]:
    tags: set[str] = set()
    for course in list_courses(db, user_id, library_type="fragmented"):
        tags.update(course_tags(course))
    for series in list_series(db, user_id):
        tags.update(decode_tags(series.tags_json))
    return sorted(tags)
